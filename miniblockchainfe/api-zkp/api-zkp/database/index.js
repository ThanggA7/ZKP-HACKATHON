...
async function getTransactionHistory(uid) {
  ...
  // Ensure uid is a number for comparison  
  const numericUid = parseInt(uid);

  console.log("Debug getTransactionHistory:");
  console.log("- Input UID:", uid, "-> Numeric:", numericUid);
  console.log(
    "- Users available:",
    users.map((u) => ({ uid: u.uid, username: u.username }))
  );

  return transactions
    .filter((t) => t.from_uid === numericUid || t.to_uid === numericUid)
    .map((t) => {
      const fromUser = users.find((u) => u.uid === t.from_uid);
      const toUser = users.find((u) => u.uid === t.to_uid);

      console.log(`Transaction ${t.id}:`);
      console.log("- from_uid:", t.from_uid, "fromUser:", fromUser?.username);
      console.log("- to_uid:", t.to_uid, "toUser:", toUser?.username);

      const result = {
        username:
          t.from_uid === numericUid
            ? toUser?.username || "Unknown"
            : fromUser?.username || "Unknown",
        amount: t.from_uid === numericUid ? -t.amount : t.amount,
